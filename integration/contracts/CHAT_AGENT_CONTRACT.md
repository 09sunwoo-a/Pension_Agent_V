...
- `customer_id`: **상담을 시작할 때 직원이 입력한 고객 식별자**입니다. 패널이 열리면 "고객 식별자를 입력해 주세요."만 표시하고, 첫 입력을 식별자로 받습니다(형식 `[A-Za-z0-9._-]{3,40}`, 예: 대화 Agent의 시연 고객 `198734-1205842`). 식별자를 정하면 그때 새 `session_id`를 만들고, 헤더의 **고객 변경**으로 다른 식별자와 새 세션으로 다시 시작합니다. 시작 화면에 추천 질문 칩은 두지 않으며, 답변 아래 `followups` 칩만 씁니다.
