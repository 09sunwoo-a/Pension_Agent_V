...
| `answer.text` | 첫 문단은 굵은 lead. 이후 문단은 본문(단일 줄바꿈 유지). `- `로 시작하는 줄 묶음은 목록(들여쓴 항목은 `· ` 접두), 큰따옴표로 감싼 문단은 **화법 카드(복사 버튼 없음, 2026-09-22)**. 꼬리의 `── 참고한 자료` / `· <유형>` 줄은 본문에서 떼어 "참고한 자료" 배지로 표시 |
| `answer.intent` | 값은 `situation`·`guide`·`agent_help`·`correction`·`lms_link`·`confirm_action`·`llm_down`. 배지는 항상 `AI 답변`(표시 분기는 보류). `confirm_action`이면서 `links`가 있으면 첫 링크를 바로 연다 |
| `answer.links[]` | `{screen, url, label}`. 본문 속 `screen`(예 `04-12-646`, 대괄호 안이어도)을 `url`로 감싼 링크로 표시하고 본문 아래 「↗ label (screen)」 줄을 둔다. URL은 Agent 값을 그대로 쓰며 프론트가 조립하지 않는다(`mystar-link://…`, 스킴은 Agent 설정) |
...
| `action` | **본문 바로 아래**(출처 블록 앞)에 `prompt`(끝의 「(네 / 아니오)」 제거)와 **네 / 아니오** 버튼. 본문 끝의 같은 문장(「— … (네 / 아니오)」)은 뗀다. `kind` 는 `lms`·`pitch`·`memo`. 쪽지(`memo`)는 본문의 ``` 초안을 버리고 `title`,`text`,`to`로 받는 사람·제목·본문 박스(복사 버튼)만 표시 |
