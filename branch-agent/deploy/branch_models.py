...
# Upper bound of the branch population (57 rows today: 42 case customers + 15 legacy demo rows).
MAX_ROWS = 64
Count = Annotated[Int, Field(le=MAX_ROWS)]
...
Ids = Annotated[list[RowId], Field(max_length=MAX_ROWS)]
...
class Clarification(Fixed):
    ...
    options: Annotated[list[Choice], Field(max_length=MAX_ROWS)]
...
class Result(Fixed):
    ...
    reasons: Annotated[list[Reason], Field(max_length=MAX_ROWS)]
...
def manifest_check(manifest):
    ...
    require(isinstance(manifest.get("row_ids"), list) and 0 < len(manifest["row_ids"]) <= MAX_ROWS, "MANIFEST")
