...
module.exports=async function(run){
 ...
 console.log(result);
 console.log(actual?'PASS: product bundle manifest matches generated manifest; remote browser uses original 48 rows':'SKIP: 03 actual manifest not generated; remote browser used validation-only manifest');
};
async function browserCheck(fixture,actual){
 ...
 function events(request,kind){
  ...
  if(final.event==='answer'){
   ...
   if(kind==='show_aggregate'||kind==='remove_condition'){
    data.intent='search';data.result={row_ids:manifest.row_ids.slice(),count:48,unknown_row_ids:[],metrics:[],reasons:[]};
 ...
 try{
  ...
  // Keep means exact list node/DOM/title preservation, even though result describes all 48.
  ...
  Object.defineProperty(c.component,'DATA',{configurable:true,value:originalData.filter(x=>x.id!=='lsm')});
