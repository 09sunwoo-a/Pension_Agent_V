...
(async()=>{
 ...
 try{
  ...
  const rec=['B04-23','B06-13','C01-10'];
  ...
  assert.equal(overview.metrics.find(x=>x.key==='irp_sum').value,7855840000);
  await new Promise(r=>setTimeout(r,7100));
  await step('isa-stat-keep',`__branchLive.send('ISA 만기 고객 몇명이야?',${JSON.stringify(rec)},true)`);
  await step('aggregate-target',`__branchLive.send('대상 고객 보기',['C01-10','B01-03','B01-22'],false,{type:'show_aggregate'})`);
  await new Promise(r=>setTimeout(r,7100));
  await step('search',`__branchLive.send('현금성 장기대기 고객 보여줘',['C01-09','jmr','jmj','B08-01','C01-01'])`);
  await new Promise(r=>setTimeout(r,7100));
  await step('add-age',`__branchLive.send('그중 40대 고객만 보여줘',['jmr','B08-01'])`);
  await new Promise(r=>setTimeout(r,7100));
  await step('add-do',`__branchLive.send('DO 미등록 조건도 추가해줘',['jmr'])`);
  await new Promise(r=>setTimeout(r,7100));
  await step('remove-age',`__branchLive.send('나이 조건 빼줘',['jmr','C01-01'])`);
