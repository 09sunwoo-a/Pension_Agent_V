...
function run(options = {}) {
  ...
  assert.deepEqual([m.record_count, m.structured_count, m.display_only_count], [48, 31, 17]);
  assert.equal(new Set(m.row_ids).size, 48);
  assert.equal(m.row_ids.includes('DEMO-01'), false);
  assert.equal(data.records.find(r => r.row_id === 'ksy').source_case_id, 'DEMO-01');
  assert.equal(data.records.filter(r => r.source_case_id === 'DEMO-01').length, 1);
  assert.equal(new Set(data.records.map(r => r.customer_id)).size, 48);
  assert.deepEqual(m.known_dates, ['2026-09-04', '2026-09-14']);
  assert.equal(m.mixed_dates, true);
  ...
  assert.equal(data.records.reduce((s, r) => s + r.irpAccount.valuationAmountKrw, 0), 5949240000);
  assert.equal(data.records.reduce((s, r) => s + (r.cash_amount ?? 0), 0), 346630000);
  assert.equal(data.records.filter(r => r.cash_amount !== null).length, 31);
  assert.equal(data.records.filter(r => r.irpAccount.valuationAmountKrw >= 70000000).length, 35);
  ...
  assert.deepEqual(data.records.filter(r => C.evaluate(r, { op: 'isa_between', start: m.as_of_date,
    end: C.dateAdd(m.as_of_date, 30) }) === true).map(r => r.row_id), ['B01-03']);
  const kim = data.records.find(r => r.row_id === 'ksy');
  assert(kim.display_overrides.includes('oneYearReturnPct'));
  assert.equal(kim.irpAccount.oneYearReturnPct, 3.1);
  ...
  console.log('PASS: 48 current rows, 31/17 source identity, nulls/overrides, IRP/cash/ISA/70M totals; repeat/key-order/input/version hash checks');
  ...
  const known = clone(kim), unknown = clone(data.records.find(r => r.source_kind === 'display-only'));
