...
def check(data):
    ...
    assert ids == manifest["row_ids"] and len(ids) == len(set(ids)) == 48
    assert "DEMO-01" not in ids
    assert sum(r["source_case_id"] == "DEMO-01" for r in records) == 1
    assert next(r for r in records if r["row_id"] == "ksy")["source_case_id"] == "DEMO-01"
    assert [r["original_order"] for r in records] == list(range(48))
    assert sum(r["source_kind"] == "structured" for r in records) == 31
    assert sum(r["source_kind"] == "display-only" for r in records) == 17
    for r in records:
    ...
    assert sum(r["irpAccount"]["valuationAmountKrw"] for r in records) == 5949240000
    amounts = [cash(r) for r in records]
    assert sum(x for x in amounts if x is not None) == 346630000
    assert sum(x is not None for x in amounts) == 31
    assert sum(r["irpAccount"]["valuationAmountKrw"] >= 70000000 for r in records) == 35
    ...
    assert [r["row_id"] for r in isa] == ["B01-03"]
    account = next(a for a in isa[0]["searchSupplement"]["externalAccounts"] if a["type"] == "ISA")
...
def main():
    ...
    print("PASS: Python data load, 48 IDs/source aliases/order/nulls, aggregates, ISA account dates, real manifest contract")
