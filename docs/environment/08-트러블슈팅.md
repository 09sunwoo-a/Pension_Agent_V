...
> 근거: `sources/KB_GenAI_ProAgent_SourceOfTruth_v2.md` §19, §40
> + `sources/STARROOT_FRONTEND_CODING_GUIDE.md` §22
...
| 증상 | 먼저 볼 것 | 문서 |
|---|---|---|
| `Couldn't find any revision to build` | remote에 tag가 push됐는가 (`git ls-remote --tags origin`) | [03](03-배포-파이프라인.md) §2 |
| `ModuleNotFoundError: No module named 'app'` | 평면 구조 import인가 / Dockerfile COPY 누락인가 | [02](02-에이전트-개발.md) §1 |
| 새로 만든 모듈이 없다고 함 | Dockerfile에 `COPY` 줄을 추가했는가 | [02](02-에이전트-개발.md) §1 |
| `pip install` 실패 | 사내 Nexus에 그 패키지/버전이 있는가 | [07](07-requirements-정책.md) |
| Pod이 `Degraded` | container startup log (import/의존성) | §2 |
| LLM 호출 401/403 | `kb-key` 헤더, stage별 API key | [02](02-에이전트-개발.md) §3 |
| LLM 호출 404 | `deployment_name` 을 endpoint path로 추측하지 않았는가 | [02](02-에이전트-개발.md) §3 ⚠️ |
| Fabrix 200인데 화면이 비어 있음 | 첫 CHUNK의 빈 `content` 를 거르는가 / 2단계 parse를 하는가 | [04](04-Fabrix-연계.md) §6, §8 |
| 블록이 빈 칸으로 렌더 | `paragraph→p`, `text→x` 변환을 했는가 | [06](06-출력계약-Contract.md) §4.3 ⚠️ |
| Browser `Failed to fetch` | Network 탭의 OPTIONS / POST / Origin / CORS 헤더 | §4 |
| `OPTIONS 405` | 그 origin에서 direct call이 차단된 것 | [04](04-Fabrix-연계.md) §9 |
| 화면이 안 뜸 | `window.PG_<파일코드>` 존재 여부 | [05](05-Starroot-프론트엔드.md) §16 |
| `Unexpected number` | `window.숫자` 로 썼는가 → bracket notation | [05](05-Starroot-프론트엔드.md) §5 |
| 공통 UI가 깨짐 | 전역 CSS를 썼는가 → `#pensionAgentDemo` namespace | [05](05-Starroot-프론트엔드.md) §6 |
| streaming 중 화면 흔들림 | 전체 DOM 재렌더 빈도 / scroll·focus 보존 | [05](05-Starroot-프론트엔드.md) §9, §11 |
| 답변에 "작성해 드립니다" 같은 메타 발화 | **Prompt 품질 문제.** 구조를 고치지 마라 | [02](02-에이전트-개발.md) §7 ⚠️ |
...
CORS 결론을 일반화하지 마라. → [04](04-Fabrix-연계.md) §9
