...
## 부점 AI 채팅창 정리 (2026-09-18)

- 헤더 부제 "고객 조회·집계·추출", 상단 "현재 화면 기준 …" 줄, 뜨지 않던 게이트 화면, 6턴 시연 가이드(패널·버튼), 첫 화면의 아이콘·설명 문장·"실제 AI 미연결" 문구, 답변의 "부점 AI" 라벨과 "현재 메인 목록 48명 · 기준일" 스탬프, "조회 취소" 버튼, 예시 질문의 G01 번호와 패널 제목, 런처의 초록 점을 제거했습니다. 하단 안내문은 "확인된 데이터만 조회해요." 한 줄로 줄였습니다.
- 답변 문장에서 "검색조건을 고객 목록에 적용했습니다 / 고객 목록은 그대로 유지했습니다" 꼬리 줄을 없앴고, "확인 필요 N명: 이름… 판단하지 않았습니다"는 "확인 필요 N명"으로 줄였습니다(골든 회귀는 답변 문장을 비교하지 않음). 남긴 것: 새 대화·최소화 버튼, "대화 기준 · …" 후속 질문 표시, 제목과 추천 질문 3개, 예시 질문·기존 목록 버튼, 오류 시 다시 시도.
- 창 높이를 540px에서 640px(뷰포트 - 118px 상한)로 키웠습니다. 위젯 API에서 setEnabled·enabled 옵션을 제거했습니다.
- 검증: `check.js`(제거 요소가 반입 JS에 없음, 창 높이, 답변 꼬리 줄 없음)·`--agent` 통과, shell 재현 브라우저 검사 통과.
