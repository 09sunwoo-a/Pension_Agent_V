...
## 부점 AI 대기·완료 UI (2026-09-18)

- 목업이 즉시 답하던 것을 세션 옵션 `latency`(어댑터 기본 2,000ms, `onParam({ branchSearchLatency: ms })`로 조정, 0이면 없음)로 늦춰 대기 상태를 보이게 했습니다. 실제 Agent를 붙이면 이 값을 0으로 두고 네트워크 대기가 그 자리를 대신합니다.
- 채팅: 대기 중 말풍선에 "조건을 해석하고 있어요 → 현재 고객 48명을 조회하고 있어요 → 결과를 정리하고 있어요" 문구가 0.7초마다 바뀌고 점 애니메이션이 붙습니다. 답변은 20ms마다 2글자(240자 초과면 4글자)씩 타이핑되며 커서가 깜빡입니다. 대기·타이핑 중에는 전송 버튼이 정지(■) 버튼이 되어 누르면 취소("요청을 취소했습니다. 목록은 유지합니다.") 또는 즉시 완성됩니다. 마지막 답변 아래에 목업 문법이 받는 후속 질문 칩(그중 잔액 2천만원 이상 / 잔액 큰 순 3명 / 그중 50대, 이미 적용된 조건은 제외, 결과 2명 미만이면 시작 질문 3개)이 붙고, 창을 닫아둔 채 답이 오면 런처에 빨간 점이 켜집니다(열면 사라짐).
- 고객 목록: 대기 중 목록 컨테이너에 `pad-branch-list--busy`로 행을 흐리게 하고 상단에 sticky 알약("부점 AI가 고객을 조회하고 있어요")을 붙입니다. 스피너는 기존 브리핑 준비 화면의 `.pad-spinner`를 그대로 씁니다. 답변이 오면 알약이 사라지고 새 행이 들어오며 제목이 노란 강조로 한 번 번쩍이고 "방금 갱신" 표시가 2.6초 뒤 사라집니다. 자동 스크롤은 넣지 않았습니다. 0명이면 제목만 `AI 검색 결과 · 0명`입니다.
- 검증: `check.js`에 latency 대기·취소 검사와 반입 JS 구성 요소 검사 추가, `--agent` 통과. shell 재현 브라우저 검사 34개 통과: 기본 2초 대기 중 문구·점·정지 버튼·스피너 알약·행 흐림, 완료 시 갱신 표시와 타이핑, 칩 표시와 클릭 시 결과 축소, 정지 버튼 취소, 최소화 중 도착 알림.
