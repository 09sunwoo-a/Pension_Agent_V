...
> **2026-09-21 모집단 변경:** 메인 목록이 57명(구조화 42 = 브리핑 30 + 대화 Agent 시연 C01 12, 표시정보 15)으로 바뀌고 기준일이 2026-09-29로 통일됐다. DEMO-01은 빌드에서 빠졌고 레거시 ksy·lsm·pjh는 C01-10·12·11로 교체됐다. 아래 48명·31/17·2026-09-14 수치는 그 이전 기록이며, 현재 값은 `branch-agent/validation/check_data.*`와 `scenarios.json`이 기준이다(IRP 7,855,840,000, 현금 935,230,000(42/15), ISA 등록 3명·30일 내 3명, ≥7천만원 41명). 행 상한은 64(`MAX_ROWS`).
...
| 실행 조회57명(변경 전 48명) | 신규 생성물 `branch-agent/deploy/branch_data.json`. Agent가 로드해 검색·집계·추천. 직접 수정 금지 |
...
시연 부점 범위는 서버의 고정 dataset_id로 현재 57명이다(2026-09-21 이전 48명). 클라이언트가 row_ids를 보내 권한 범위를 늘리게 하지 않는다. `x_client_user`는 감사 식별일 뿐 실제 인증된 부점 권한의 대체물이 아니다. 실제 직원/부점별 데이터 연결은 사내 인증 컨텍스트가 확보된 뒤 추가한다.
