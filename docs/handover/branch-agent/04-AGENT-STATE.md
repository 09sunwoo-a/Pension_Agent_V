...
- 현재 기준일(2026-09-29, 이전 2026-09-14)에 확인된 근거만 추천. 오래된17명 정보를 이유로 “관리 필요 없음”으로 단정하지 않는다.
...
null State는 “확정된 AI 결과 없음”이다. 첫 “그중”은 전체 목록(현재 57명)으로 추측하지 않는다. 01에 정의한 **State.active**를 사용한다. 검색/추천/대상보기 후 true, 기본/복원 후 false. active=false에서는 후속 범위를 물어본다. 처음 통계만 본 뒤의 “그중”도 명확화 후 집계 대상으로 연결한다.
