...
const VERSIONS = Object.freeze({ dataset_id: 'branch-demo.v1', rule_version: 'branch-rules.v1',
  projection_version: 'current-data.v1', as_of_date: '2026-09-29' });
...
function fromProjection(source, versions = VERSIONS) {
  ...
  // Population = the current main list: every case customer (structured) plus the visible legacy demo rows (display-only).
  const meta = source.metadata;
  if (!records.length || new Set(rowIds).size !== records.length || records.length !== meta.recordCount ||
    structured.length !== meta.structuredCount || displayOnly.length !== meta.displayOnlyCount || !structured.length)
    throw new Error('Branch population must equal the projected main list (' + meta.recordCount + ' rows: ' + meta.structuredCount + ' structured / ' + meta.displayOnlyCount + ' display-only)');
  if (rowIds.includes('DEMO-01') || records.some(row => row.source_case_id === 'DEMO-01' || row.row_id === 'ksy'))
    throw new Error('DEMO-01 and the replaced legacy 김서연 row must not appear');
  if (new Set(structured.map(row => row.source_case_id)).size !== structured.length ||
