...
async function check(source){
 source=clone(source);const original=JSON.stringify(source),engine=E.create(source),rec=['B04-23','B06-13','B01-03'],idle=['lsm','jmr','jmj','B08-01'];
 ...
 send('그중 50대 고객만 보여줘');ids(['lsm']);
 send('DO 미등록 조건도 추가해줘');ids(['lsm']);assert.equal(state.conditionChips.length,3);
 let r=send('이 고객 브리핑해줘');assert.match(r.answer,/51세/);assert(!/10주|1천만원/.test(r.answer));ids(['lsm']);
 const ageChip=state.conditionChips.find(c=>c.label.includes('연령'));
 const beforeRemove=clone(state);send('나이 조건 빼줘');ids(['lsm','jmr']);
 ...
 r=send('납입금 미운용 고객 찾아주고 현금 합계도 알려줘');ids(['B02-04','B02-27']);assert.equal(r.metrics.cashAmountKrw,2550000);
 r=send('IRP 잔액 7천만원 이상 고객 보여줘');assert.equal(state.mainListCaseIds.length,35);assert(!source.records.some(c=>r.answer.includes(c.customer.name)));
 // H: aggregates never overwrite the current list; target actions do.
 recommend();const title=state.listTitle;
 r=send('오늘 부점 현황 말해줘');ids(rec);assert.equal(state.listTitle,title);assert.equal(r.metrics.customerCount,48);assert.equal(r.metrics.irpAmountKrw,5949240000);
 r=send('우리 부점 현금성자산 합계는 얼마야?');ids(rec);assert.equal(r.metrics.cashAmountKrw,346630000);assert.equal(r.metrics.cashUnknownCount,17);
 r=send('현금성 장기대기 고객은 몇 명이야?');ids(rec);assert.equal(r.matchedCount,4);
 send(r.actions[0].label,r.actions[0].command);ids(idle);send('그중 50대 고객만 보여줘');ids(['lsm']);
 r=send('납입금 미운용 고객 몇 명이고 현금 합계 얼마야?');ids(['lsm']);assert.equal(r.metrics.cashAmountKrw,2550000);
 r=send('ISA 만기 고객 몇명이냐?');ids(['lsm']);assert.equal(r.matchedCount,3);
 send(r.actions[0].label,r.actions[0].command);assert.deepEqual([...state.mainListCaseIds].sort(),['B01-03','B01-22','ksy']);
 // R/B: evidence, facts, sums, named briefings and restore.
 recommend();
 for(const [q,pattern] of [['정미경 간단히 브리핑해줘',/6,870만원.*수수료/],['신경호는 어떻게 관리하면 돼?',/1억 6,470만원.*10일/],['오민서는 왜 관리해야 해?',/10월 7일.*23일/]]){r=send(q);assert.match(r.answer,pattern);ids(rec);assert.equal(r.uiEffect,'answer_only_keep_list');}
 send('그중 퇴직금 운용 미지시 고객만 보여줘');ids(['B06-13']);send('처음 추천한 고객 다시 보여줘');ids(rec);
 r=send('이 3명의 IRP 잔액 합계는?');ids(rec);assert.equal(r.metrics.irpAmountKrw,296900000);
 r=send('ISA 만기가 가까운 고객은 누구고 언제 만기야?');ids(rec);assert.match(r.answer,/오민서.*23일/);
 send('그중 ISA 만기 고객만 보여줘');ids(['B01-03']);r=send('이 고객의 ISA 만기자금은 얼마야?');assert.match(r.answer,/미확인|확인되지/);assert.match(r.answer,/별도 계좌/);
 ...
 send('현금 많은 고객');ids(rec);send('금액');ids(rec);r=send('500만원 이상');assert.equal(r.matchedCount,11);assert.equal(r.unknownCount,17);
 ...
 r=send('운용금액 7천만원 이상 고객 보여줘');assert.equal(r.resultStatus,'clarification_required');r=send(r.actions[0].label,r.actions[0].command);assert.equal(r.matchedCount,35);
 send('앞으로 30일 이내 ISA 만기 고객을 보여주고, 확인된 ISA 평가금액도 알려줘.');ids(['B01-03']);
 r=send('오늘 ISA 만기 고객은 몇 명이야?');ids(['B01-03']);send(r.actions[0].label,r.actions[0].command);ids([]);
