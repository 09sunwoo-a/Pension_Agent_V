...
**LLM을 호출하지 않고, 화면에서 선택한 고객의 저장된 S1–S5 브리핑을 실제 사내 Agent/FabriX 경로로 받아 표시하는지 확인합니다.** 문장은 기존 더미와 같을 수 있습니다. API 수신 상태와 실제 Network 요청으로 연동 성공을 판단합니다.
...
- [ ] 기존 김서연 화면, 구조화 김서연 `DEMO-01`, 30개 사례가 표시되는지 확인합니다.
- [ ] 상단 고객정보/IRP/보유상품, S3 추천상품 펼침, S4 고객 반응, S5 TIP/업무를 확인합니다. 상담 후 확인 기록은 없습니다.

이 단계는 저장된 로컬 내용으로 확인합니다. 아직 API 버튼이 성공하지 않아도 Agent 배포 전이라면 정상입니다. 우측 실시간 상담은 이번 S1–S5 API 대상이 아닙니다.
...
설정은 메모리에만 보관되므로 새로고침/화면 종료 후 다시 입력해야 합니다. 소스·문서·콘솔 기록에 인증값을 넣지 않습니다. Network에는 헤더가 보이므로 HAR/스크린샷/전체 요청을 공유할 때 비밀값과 식별정보를 포함하지 않습니다.

- [ ] **대표 · 상품 제안 · 김서연 (`DEMO-01`)**을 선택합니다. 원래 김서연 `ksy`는 API 호출 대상이 아닙니다.
- [ ] **실제 브리핑 호출**을 누릅니다.
...
- [ ] 화면 상태가 **실제 API 응답 · 내용 검토 전 / 정상 수신**으로 바뀌는지 확인합니다. 브리핑 문장이 기존과 같은 것은 고정 반환 단계의 정상 동작입니다.
- [ ] 상단 고객/계좌/보유상품이 API 응답 때문에 바뀌지 않는지 확인합니다.
- [ ] B01-22, B06-13, 다른 사례로 바꿔 각 고객에 맞는 결과를 받는지 확인합니다.
...
- [ ] 오류가 나면 기존 브리핑을 유지하고 오류 안내를 표시합니다. 더미로 복원/재요청/설정 삭제도 확인합니다.
